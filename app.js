...
async function openChartModal(symbol) {
  ...
  const sourceLabel = data.source === 'yahoo' ? 'Yahoo Finance (live)' : data.source === 'stooq' ? 'Stooq (live)' : 'Lokal fallback';
  const sourceText = `${sourceLabel}${data.liveError ? ` (${data.liveError})` : ''}`;
